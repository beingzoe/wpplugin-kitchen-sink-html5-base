 ...
 *
 * This is CakePHP's inflector class updated to PHP5 syntax for Vork
 *
 ...
 * Pluralize and singularize English words.
 *
 * inflectorComponent pluralizes and singularizes English nouns.
 ...
 * Typical usage:
 *
 *      $inflector = get::component('inflector');
 *
 ...
 *      //prints: person
 *
 *      echo $inflector->camelize('cakephp_inflector_class');
 *      //prints: cakephpInflectorClass
 *
 *      echo $inflector->underscore('cakephpInflectorClass');
 *      //prints: cakephp_inflector_class
 */
class inflectorComponent {
    ...
    */
    ...
    /**
    * Gets a reference to the inflectorComponent object instance
    *
    * @return object
    * @access public
    */
	public function &getInstance() {
		...
		if (!$instance) {
			$instance[0] = new inflectorComponent();
		}
		return $instance[0];
	}
     ...
     *
     * {{{
     * inflectorComponent::rules('plural', array('/^(inflect)or$/i' => '\1ables'));
     * inflectorComponent::rules('plural', array(
     *     'rules' => array('/^(inflect)ors$/i' => '\1ables'),
     *     'uninflected' => array('dontinflectme'),
     *     'irregular' => array('red' => 'redlings')
     * ));
     * inflectorComponent::rules('transliteration', array('/å/' => 'aa'));
     * }}}
     *
	...
	public function rules($type, $rules, $reset = false) {
		$_this =& inflectorComponent::getInstance();
		$var = '_'.$type;
	...
	public function camelize($lowerCaseAndUnderscoredWord) {
		...
		if (!($result = $_this->_cache(__FUNCTION__, $lowerCaseAndUnderscoredWord))) {
			$result = str_replace(' ', '', inflectorComponent::humanize($lowerCaseAndUnderscoredWord));
	...
	public function tableize($className) {
		...
		if (!($result = $_this->_cache(__FUNCTION__, $className))) {
			$result = inflectorComponent::pluralize(inflectorComponent::underscore($className));
	...
	public function classify($tableName) {
		...
		if (!($result = $_this->_cache(__FUNCTION__, $tableName))) {
			$result = inflectorComponent::camelize(inflectorComponent::singularize($tableName));
	...
	public function variable($string) {
		...
		if (!($result = $_this->_cache(__FUNCTION__, $string))) {
			$string2 = inflectorComponent::camelize(inflectorComponent::underscore($string));
     ...
     * @deprecated $map param will be removed in future versions. Use inflectorComponent::rules() instead
     * @return string
     * @access public
     * @static
     * @link http://book.cakephp.org/view/1479/Class-methods
